...
@section('content')
    <div class="max-w-3xl mx-auto">
        ...
        <div class="flex items-center justify-between gap-4 mb-8">
            <div>
                <h1 class="text-2xl font-extrabold text-primary">Pengumuman</h1>
        ...
        <div class="bg-white rounded-2xl shadow-soft divide-y divide-gray-100">
                ...
                <div class="p-5 flex items-center justify-between gap-4">
                    <div class="min-w-0">
                        <div class="flex items-center gap-2 mb-1">
                            <span class="px-2 py-0.5 rounded-full text-xs font-semibold {{ $announcement->status->value === 'published' ? 'bg-secondary/10 text-secondary' : 'bg-gray-100 text-gray-500' }}">
                    ...
                    <div class="flex items-center gap-3 shrink-0">
                        <a href="{{ route('organizations.announcements.edit', [$organization, $announcement]) }}{{ $builderQuery }}"
                           class="text-primary text-sm font-semibold hover:underline">Edit</a>
                        <form action="{{ route('organizations.announcements.destroy', [$organization, $announcement]) }}" method="POST"
                              x-data @submit.prevent="if (await confirmAction('Hapus pengumuman ini?')) $el.submit()">
                            @csrf
                            @method('DELETE')
                            @if ($fromBuilder)
            ...
            @empty
                <p class="p-8 text-center text-sm text-gray-400">Belum ada pengumuman. Tambahkan yang pertama.</p>
            @endforelse
        </div>
    </div>
@endsection
