...
|---|---|---|
| Authentication dan onboarding organisasi | Sebagian (checklist diperluas 2026-08-17) | Login/register bawaan Laravel Breeze-style ada (`resources/views/auth`). Alur pilih-tipe-lalu-buat sudah ada dari sesi sebelumnya (§24.1 baris Template). Checklist "Langkah Awal" di dashboard organisasi (`Organization::onboardingChecklist()`) kini **3 item** (Atur Brand, Isi Kontak, Publish Situs) — item "Isi Kontak" baru ditambah sesi ini (lihat §24.4), item "Tambah Pengurus" yang sempat ada juga di sesi yang sama sengaja **dihapus lagi** atas permintaan eksplisit: pengurus sudah bisa dikelola langsung dari dalam builder lewat link "Kelola Pengurus →" di section `struktur-pengurus`, jadi tidak perlu jadi langkah setup terpisah yang menuntun keluar builder. Status selesai tetap dihitung otomatis dari data yang sudah ada, bukan tabel progress baru. **Bukan** wizard multi-step terpisah — checklist muncul di dashboard existing setelah organisasi dibuat, tidak mengubah form create. Item "Publish Situs" sengaja ditampilkan non-interaktif "Segera hadir" karena mekanisme publish sungguhan belum ada (lihat baris Subdomain publishing) |
...
| Brand settings / guided design system dasar | Sebagian (warna diterapkan ke canvas 2026-08-17) | Migration `organizations.primary_color`/`secondary_color`/`logo`. Halaman standalone `organizations/{org}/brand` (`OrganizationBrandController`) dengan color picker native + text input hex + picker logo (reuse media library) + pratinjau langsung. **Auto-copy dari template saat organisasi dibuat**: `StoreOrganizationRequest::prepareForValidation()` menyalin `template.structure.brand.primary/secondary` ke `organizations.primary_color/secondary_color` kalau belum diisi eksplisit — sesuai PRD §6. `Organization::primaryColor()`/`secondaryColor()` jadi effective-value accessor dengan fallback chain: kolom sendiri → `template.structure.brand` → default platform (`#2C368B`/`#079C4E`) — pola sama persis dengan `templates/preview.blade.php`. **Warna kini benar-benar terlihat efeknya di canvas builder** (sejak sesi iframe refactor, lihat §24.4) — sebelumnya cuma tersimpan di DB tanpa efek visual sama sekali. **Logo kini juga dipakai** (sesi 2026-08-17, lihat §24.4): `header.blade.php`/`footer.blade.php` menampilkan `organizations.logo` sebagai `<img>` menggantikan inisial nama saat logo terisi, fallback ke inisial saat kosong. **Logo juga dipakai sebagai favicon, dan gambar hero sebagai og:image** (sesi terpisah tanggal yang sama, lihat §24.4) di `organizations/builder/canvas.blade.php` — satu-satunya dokumen HTML lengkap (`<head>` sungguhan) untuk situs milik organisasi yang ada saat ini, karena belum ada route publik tenant (subdomain publishing masih "Belum mulai"). Font pairing, gaya tombol, dan border radius dari PRD §6 belum dikerjakan — scope sengaja dibatasi warna+logo |
