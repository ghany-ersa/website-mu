...
- **Mobile-first, always.** Design and build every UI for mobile screens first, then progressively enhance up to tablet/desktop (e.g. Tailwind's unprefixed utilities target mobile, add `sm:`/`md:`/`lg:` overrides for larger screens). This applies to new pages, components, and the drag-and-drop editor/template output alike - never design desktop-first and retrofit mobile styles.
